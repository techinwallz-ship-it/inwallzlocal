...
export const removePlaylistFromGroup = (groupId, playlistId) =>
  ...
  const res = await fetch(
    `http://localhost:5000/api/display/dashboard-stats?userId=${userId}`
  );
...
export const addPlayerToGroup = async (groupId, displayId) => {
  const res = await fetch(
    `http://localhost:5000/api/groups/${groupId}/players`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
export const runGroup = async (groupId, playlistId) => {
  return fetch(
    `http://localhost:5000/api/groups/${groupId}/run`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ playlistId })
...
export const getPlaylistStats = async (playlistId) => {
  const res = await fetch(
    `http://localhost:5000/api/playlists/${playlistId}/stats`
  );
  return res.json();
};
